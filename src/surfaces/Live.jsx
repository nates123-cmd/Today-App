...
import { PILLARS, PLACED_BLOCKS, SUGGESTIONS, OPEN_TASKS, TODAY_DATE } from '../data.js'
...
export function Live({ nowHour = 11, nowMinute = 14, placed: placedProp, onOpenBlock }) {
  ...
  const dateStr = TODAY_DATE.toLocaleDateString('en-US',
          ...
          {visibleOpen.map(t => (
            <div className={`open-task ${openDone.has(t.id) ? 'done' : ''}`} key={t.id}>
              <button className={`open-task-check ${openDone.has(t.id) ? 'checked' : ''}`}
                ...
                {openDone.has(t.id) && <IconCheck w={10} />}
              </button>
